...
void print_progress_json(std::ostream& output, const cube::NativeSearchProgress& progress) {
    output << "{\"type\":\"progress\",\"lower_bound\":" << progress.lower_bound
           ...
           << ",\"iteration_split_nodes\":" << progress.iteration_split_nodes
           << ",\"nodes\":" << progress.total_nodes
           << ",\"split_nodes\":" << progress.total_split_nodes
           << ",\"transposition_hits\":" << progress.transposition_hits
           << ",\"tail_queries\":" << progress.tail_queries
           << ",\"tail_bloom_rejects\":" << progress.tail_bloom_rejects
           << ",\"tail_exact_queries\":" << progress.tail_exact_queries
           << ",\"tail_probes\":" << progress.tail_probes
           << ",\"tail_hits\":" << progress.tail_hits
...
void print_result_json(
    std::ostream& output,
    const cube::NativeSolveResult& result,
    const cube::NativeOptimalSolver& solver,
    bool framed = false) {
    output << "{\"ok\":true";
    if (framed) output << ",\"type\":\"result\"";
    ...
    output << ",\"solution\":\"" << moves_text(result.moves)
           << "\",\"depth\":" << result.depth
           ...
           << ",\"tail_queries\":" << result.tail_queries
           << ",\"tail_bloom_rejects\":" << result.tail_bloom_rejects
           << ",\"tail_exact_queries\":" << result.tail_exact_queries
           << ",\"tail_probes\":" << result.tail_probes
    ...
    try {
        ...
        if (command == "build-tail-pdb") {
            ...
            for (int index = 3; index < argc; ++index) {
                ...
                else if (option == "--threads" && index + 1 < argc) threads = std::stoi(argv[++index]);
                else if (option == "--force") force = true;
                else throw std::invalid_argument("unknown build option: " + option);
            }
        ...
        if (command == "serve") {
            ...
            if (!phase1_pdb_path.empty()) solver.load_phase1_pdb(phase1_pdb_path);
            if (!pdb_path.empty()) solver.load_corner_pdb(pdb_path);
            for (int group = 0; group < 8; ++group) {
                if (!edge_pdb_paths[group].empty()) solver.load_edge_pdb(group, edge_pdb_paths[group]);
            }
            if (!tail_pdb_path.empty()) solver.load_tail_database(tail_pdb_path);
            ...
            while (std::getline(std::cin, request)) {
                try {
                    ...
                    if (fields.size() != 5) throw std::invalid_argument("invalid serve request");
                    ...
                    std::cout << "{\"ok\":false,\"type\":\"error\",\"error\":\""
                              << error.what() << "\"}\n" << std::flush;
                }
            }
            return 0;
        ...
        if (command == "solve") {
            ...
            for (int index = 3; index < argc; ++index) {
                ...
                else if (option == "--transposition") options.use_transposition = true;
                else if (option == "--no-transposition") options.use_transposition = false;
                else if (option == "--no-direction-probe") options.use_direction_probe = false;
            ...
            const std::array<std::filesystem::path, 4> first_edge_pdb_paths{
                edge_pdb_a_path, edge_pdb_b_path, edge_pdb_c_path, edge_pdb_d_path};
            for (int group = 0; group < 4; ++group) {
                const auto& path = first_edge_pdb_paths[group];
                if (!path.empty()) {
                    if (!std::filesystem::exists(path)) throw std::invalid_argument("edge PDB does not exist");
        ...
        std::cout << "{\"ok\":true,\"facelets\":\"" << cube::to_facelets(state)
                  << "\",\"inverse_facelets\":\"" << cube::to_facelets(state.inverse())
