...
std::uint64_t checksum_bytes_parallel(const std::uint8_t* data, std::uint64_t size) {
    ...
    if (chunk_count == 0) return checksum_bytes(data, size);
    std::vector<std::uint64_t> hashes(chunk_count);
    std::atomic<std::size_t> cursor{0};
    const int thread_count = std::clamp(
        static_cast<int>(std::thread::hardware_concurrency()), 1, 32);
    ...
    for (std::size_t chunk = 0; chunk < hashes.size(); ++chunk) {
        result = mix_checksum(result ^ hashes[chunk] ^
            (static_cast<std::uint64_t>(chunk) * 0x9E3779B97F4A7C15ULL));
    }
    return result;
}

void normalize_empty_keys(
    std::uint64_t* keys,
    ...
    for (int thread = 0; thread < threads; ++thread) {
        workers.emplace_back([&, thread] {
            ...
            for (std::uint64_t slot = begin; slot < end; ++slot) {
                if (((metadata[slot] >> 16U) & 0xFFU) == kEmpty) keys[slot] = 0;
            }
        });
    }
    for (auto& worker : workers) worker.join();
...
std::uint64_t bloom_word_count(std::uint64_t slot_count) noexcept {
    return slot_count / 16U;
}

void bloom_locations(
    PackedKey key,
...
bool bloom_maybe_contains(const std::uint64_t* bloom, std::uint64_t word_count, PackedKey key) noexcept {
    ...
    for (int index = 0; index < kBloomHashes; ++index) {
        if ((bloom[words[index]] & masks[index]) == 0) return false;
    }
    return true;
}

std::unique_ptr<std::uint64_t[]> build_bloom_filter(
...
void require_tail_build_memory(int depth) {
    ...
    if (!GlobalMemoryStatusEx(&status)) throw windows_error("GlobalMemoryStatusEx");
...
std::uint8_t metadata_move(std::uint32_t metadata) noexcept {
    return static_cast<std::uint8_t>(metadata >> 24U);
}
...
std::uint32_t pack_metadata(PackedKey key, std::uint8_t distance, std::uint8_t move_to_goal) noexcept {
    return static_cast<std::uint32_t>(key.high) |
        (static_cast<std::uint32_t>(distance) << 16U) |
        (static_cast<std::uint32_t>(move_to_goal) << 24U);
}

std::uint64_t find_compact_slot(
    const std::uint64_t* keys,
    const std::uint32_t* metadata,
    std::uint64_t slot_count,
    ...
    while (true) {
        ...
        if (metadata_distance(value) == kEmpty ||
            (keys[index] == key.low && metadata_key_high(value) == key.high)) {
            return index;
        }
        index = (index + 1) & mask;
    }
...
bool insert_compact_concurrent(
    std::uint64_t* keys,
    std::uint32_t* metadata,
    std::uint64_t slot_count,
    PackedKey key,
    std::uint8_t distance,
    std::uint8_t move_to_goal) noexcept {
    const std::uint64_t mask = slot_count - 1;
    std::uint64_t index = hash_key(key) & mask;
    while (true) {
        ...
        if (keys[index] == key.low && metadata_key_high(value) == key.high) return false;
        index = (index + 1) & mask;
    }
}

const LegacyTailEntry* find_legacy_slot(
    const LegacyTailEntry* entries,
    std::uint64_t mask,
    PackedKey key) noexcept {
    std::uint64_t index = hash_key(key) & mask;
    while (true) {
...
std::optional<TailHit> find_compact_entry(
    const std::uint64_t* keys,
    const std::uint32_t* metadata,
    std::uint64_t mask,
    PackedKey key,
    TailLookupCounters* counters) noexcept {
    std::uint64_t index = hash_key(key) & mask;
    while (true) {
        if (counters != nullptr) ++counters->probes;
        const std::uint32_t value = metadata[index];
        if (metadata_distance(value) == kEmpty) return std::nullopt;
...
TailDatabase::TailDatabase(const std::filesystem::path& path) {
    ...
    const std::uint64_t bloom_bytes = bloom_format
        ? bloom_word_count(header->slot_count) * sizeof(std::uint64_t)
        : 0;
    ...
    if (size.QuadPart == static_cast<LONGLONG>(sizeof(TailHeader) + data_bytes)) {
        if (bloom_format && header->reserved[2] == kBloomChecksumMarker) {
            ...
            const std::uint64_t metadata_checksum = checksum_bytes_parallel(
                metadata, header->slot_count * sizeof(std::uint32_t));
            const std::uint64_t bloom_checksum = checksum_bytes_parallel(bloom, bloom_bytes);
            checksum_valid =
                header->reserved[0] == checksum_bytes_parallel(
                    keys, header->slot_count * sizeof(std::uint64_t)) &&
            ...
            checksum_valid =
                header->reserved[0] == checksum_bytes_parallel(
                    keys, header->slot_count * sizeof(std::uint64_t)) &&
                header->reserved[1] == checksum_bytes_parallel(
                    metadata, header->slot_count * sizeof(std::uint32_t));
        } else {
            checksum_valid = header->reserved[0] == checksum_bytes(
                view_ + sizeof(TailHeader), data_bytes);
        }
    }
    const bool valid =
        header->magic == kMagic && (legacy || compact || bloom_format) &&
        header->header_size == sizeof(TailHeader) &&
        header->metric == 1 && header->depth <= 7 && header->slot_count > 0 &&
...
std::optional<TailHit> TailDatabase::lookup(
    const CubieCube& cube,
    TailLookupCounters* counters) const noexcept {
    if (counters != nullptr) ++counters->queries;
    const PackedKey key = pack_cube(cube);
    if (version_ == kBloomVersion && !bloom_maybe_contains(bloom_, bloom_word_count_, key)) {
        if (counters != nullptr) ++counters->bloom_rejects;
        return std::nullopt;
    }
    if (counters != nullptr) ++counters->exact_queries;
    if (version_ == kCompactVersion || version_ == kBloomVersion) {
        const auto* keys = static_cast<const std::uint64_t*>(entries_);
        const auto* metadata = reinterpret_cast<const std::uint32_t*>(keys + slot_count_);
        auto result = find_compact_entry(keys, metadata, mask_, key, counters);
        if (result.has_value() && counters != nullptr) ++counters->hits;
        return result;
    }
...
void build_tail_database(const std::filesystem::path& path, int depth, int threads, bool force) {
    if (depth < 0 || depth > 7) throw std::invalid_argument("tail database depth must be 0..7");
    if (!force && std::filesystem::exists(path) && valid_existing_file(path, depth)) return;
    std::filesystem::create_directories(path.parent_path());
    require_tail_build_memory(depth);
    threads = std::clamp(
        threads > 0 ? threads : static_cast<int>(std::thread::hardware_concurrency()), 1, 64);
    ...
    for (int current_depth = 0; current_depth < depth; ++current_depth) {
        ...
        const bool parallel_layer = threads > 1 &&
            (!frontier_shards.empty() || frontier.size() >= 1024);
        if (parallel_layer) {
            std::atomic<std::size_t> cursor{0};
            ...
            for (int thread = 0; thread < threads; ++thread) {
                workers.emplace_back([&, thread] {
                    ...
                    if (!final_layer) {
                        const std::size_t input_size = frontier_shards.empty()
                            ? frontier.size()
                            : frontier_shards[thread].size() * threads;
                    ...
                    auto expand_state = [&](const CubieCube& state) {
                        for (int move = 0; move < 18; ++move) {
                            ...
                            if (insert_compact_concurrent(
                                    ...
                                    inverse_move(move))) {
                                ++local_count;
                                if (!final_layer) output.push_back(child);
                            }
                        }
                    };
                    if (!frontier_shards.empty()) {
                        for (const CubieCube& state : frontier_shards[thread]) expand_state(state);
                    } else {
                        while (true) {
                            ...
                            for (std::size_t item = begin; item < end; ++item) expand_state(frontier[item]);
                        }
                    }
            ...
            for (auto& worker : workers) worker.join();
            for (const std::uint64_t count : local_counts) next_count += count;
            if (!final_layer) {
            ...
            if (!final_layer) next.reserve(frontier.size() * 12);
            for (const CubieCube& state : frontier) {
                for (int move = 0; move < 18; ++move) {
                    CubieCube child = state.apply_move(move);
                    const PackedKey key = pack_cube(child);
                    const std::uint64_t slot = find_compact_slot(
                        keys.get(), metadata.get(), slot_count, key);
                    if (metadata_distance(metadata[slot]) != kEmpty) continue;
                    keys[slot] = key.low;
                    metadata[slot] = pack_metadata(
                        key,
                        static_cast<std::uint8_t>(current_depth + 1),
                        inverse_move(move));
                    ++next_count;
                    if (!final_layer) next.push_back(std::move(child));
                }
            }
        }
        frontier = std::move(next);
        if (final_layer) frontier_shards.clear();
        state_count += next_count;
        if (state_count * 10 >= slot_count * 7) {
            throw std::runtime_error("tail database hash table exceeded 70% load");
        }
        std::cerr << "tail-db depth=" << current_depth + 1 << " frontier=" << next_count
                  << " states=" << state_count << "\n";
    }
    ...
    header.reserved[0] = checksum_bytes_parallel(
        reinterpret_cast<const std::uint8_t*>(keys.get()), slot_count * sizeof(keys[0]));
    const std::uint64_t metadata_checksum = checksum_bytes_parallel(
        reinterpret_cast<const std::uint8_t*>(metadata.get()),
        slot_count * sizeof(metadata[0]));
    const std::uint64_t bloom_checksum = checksum_bytes_parallel(
        reinterpret_cast<const std::uint8_t*>(bloom.get()),
        bloom_words * sizeof(bloom[0]));
    header.reserved[1] = mix_checksum(metadata_checksum ^ std::rotl(bloom_checksum, 17));
